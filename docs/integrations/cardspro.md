...
- **`CARD_ISSUE` не создаёт карту автоматически.** В теле этого колбэка нет ни `user_id`, ни кода
  продукта — только `san`, `docid`, `request_id`, `amount`, `currency`. Чтобы автоматически привязать
  выпущенную карту к пользователю, нужно на своей стороне до вызова `issueCard()` сохранять
  `request_id` вместе с будущим владельцем карты (например, в отдельном поле `cards.pending_request_id`),
  а по приходу `CARD_ISSUE` находить карту по этому `request_id`, а не по `san` (`san` появляется только
  в момент выпуска). Эта логика пока не реализована — событие только логируется.
