...
| `OTP_CODE` | Находит карту, шлёт владельцу уведомление с кодом (`otpCode`) — без собственного хранилища, вкладка «3DS коды» в ЛК запрашивает их напрямую через `GET /{san}/otp-codes` |
| `CARD_ISSUE`, `EXTRA_FEE_CARD`, `EXTRA_FEE_CAP`, `KYC_CHANGE` | **Только логируются** в `ProviderMessage` — см. «Известные ограничения» ниже |
...
- **`CARD_ISSUE` активирует карту, только если выпуск был инициирован через `card_provider_operations`.**
  В теле этого колбэка нет ни `user_id`, ни кода продукта — только `san`, `docid`, `request_id`,
  `amount`, `currency`, но это уже не проблема: `Card` заводится до оплаты (ещё до вызова
  `issueCard()`), и та часть админки/API, которая вызывает `issueCard()`, заводит
  `CardProviderOperation` с уже известным `card_id` (`type=issue`, `request_id`,
  `payload=['topup_usd' => ...]` — сумма начального пополнения нужна по приходу
  подтверждения для расчёта комиссии провайдера). Владелец и карточный продукт больше
  не нужно снепшотить в `payload` — они уже есть у самой `Card` (`card_id` операции).
  Самого такого админ-действия/API-метода оформления заказа в панели пока нет — это
  следующий шаг (см. `CARD_ORDER_AND_ISSUANCE_FLOW.md` в корне репозитория). Без такой
  записи событие по-прежнему только логируется.
...
- **`OTP_CODE`** — коды одноразовые и короткоживущие, отдельного хранилища под них в схеме нет. Обрабатывается только для
  уведомления ({@see \App\Services\Integrations\CardsPro\CardsProWebhookHandler::handleOtpCode()} →
  `Notification::notify(..., NotificationEvent::OtpCodeReceived, ['code' => $otpCode])`), а сам список
  кодов во вкладке «3DS коды» в ЛК всегда запрашивается у провайдера напрямую.
...
не дойти или прийти с задержкой. Поэтому есть шесть универсальных (не привязанных к
...
| `providers:sync-merchant-rates` | Опрашивает `GET /products/search-by-merchant` по каждому мерчанту из справочника `Merchant` и сохраняет рейтинг успешных платежей каждого карточного продукта в `MerchantProductRate` (по `provider_product_code`). Матрица по активным продуктам — админка «Настройки» -> «Рейтинг платежей по мерчантам» | 1 раз в сутки |
...
- `card_provider_operations` — таблица асинхронных операций (`issue`/`topup`/`withdraw`/
  `block`), которые мы инициировали и ждём финального статуса. `card_id` заполнен с
  самого начала (`Card` заводится до оплаты, в шаге 0 оформления заказа); для `issue`
  в `payload` хранится `topup_usd` — сумма начального пополнения, нужная при активации
  карты для расчёта комиссии провайдера (`CardProviderOperationResolver::recordIssueExpenses()`).
  Закрывается либо вебхуком (`CARD_ISSUE` — см. «Известные ограничения»), либо
  `providers:sync-pending-operations`.
