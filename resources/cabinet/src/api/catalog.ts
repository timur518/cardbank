...
// Каталог карточных продуктов (CardProductController::index()) для выбора при
// оформлении заявки на выпуск карты.
export async function fetchCardProducts(): Promise<CardProduct[]> {
    const { data } = await apiClient.get<{ data: CardProduct[] }>('/card-products');
    return data.data;
}

// Способы оплаты (PaymentMethodController::index()) для шага оплаты при выпуске
// карты и при пополнении баланса.
