...
enum NotificationEvent
{
    ...
    public function title(array $params = []): string
    {
        return match ($this) {
            ...
            self::PasswordResetRequested => 'Новый пароль отправлен на почту',
        };
    }
    ...
    public function body(array $params = []): string
    {
        return match ($this) {
            ...
            self::PasswordResetRequested => "Мы отправили новый пароль на {$params['email']}. Рекомендуем сменить его на свой после входа в личный кабинет.",
        };
    }
    ...
    private function purchaseDeclinedBody(array $params): string
    {
        ...
        return $merchant
            ? "Покупка на {$params['amount']} в «{$merchant}» отклонена. Проверьте баланс карты или лимиты и попробуйте снова."
            : "Покупка на {$params['amount']} по карте •••• {$params['last4']} отклонена. Проверьте баланс карты или лимиты и попробуйте снова.";
    }
