...
### Новое поле, которого пока нет в БД

`users.personal_data_consent_at` (timestamp, nullable) — момент согласия на обработку
персональных данных при регистрации. Сейчас нигде не хранится (в `LegalDocument`
только версии текста документов, а не факт согласия конкретного пользователя).
Нужна миграция перед реализацией метода регистрации (п. 2).
